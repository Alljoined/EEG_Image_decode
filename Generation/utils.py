    ...
    for data_iter_step, (data_dcit) in enumerate(data_loader):
        ...
        cor = torch.mean(
            torch.tensor(
                [
                    torch.corrcoef(
                        torch.cat([p[0].unsqueeze(0), s[0].unsqueeze(0)], axis=0)
                    )[0, 1]
...
def interpolate_pos_embed(model, checkpoint_model):
    if "pos_embed" in checkpoint_model:
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.reshape(-1, orig_size, embedding_size).permute(
                0, 2, 1
            )
...
def create_dataset(sub, config, train):
    return EEGDataset(
        config["data_path"],
        subjects=[sub] if config["insubject"] else [],
        exclude_subject=sub if not config["insubject"] else None,
        train=train,
    )


def create_dataloader(dataset, batch_size, shuffle, num_workers=0, drop_last=False):
    return DataLoader(
        dataset,
...
class wandb_logger:
    ...
    def load(self, net):
        path = os.path.join(
            self.config["path_data"], self.config["path_ckpt"], self.config["file_ckpt"]
        )
